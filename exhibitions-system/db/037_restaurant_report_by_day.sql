-- ============================================================
-- تحليلات المطعم: إضافة سلسلة المبيعات اليومية (by_day) | Migration 037
-- لدعم رسم اتجاه المبيعات اليومي في صفحة التحليلات.
-- ============================================================
...
begin
  ...
  return json_build_object(
    'summary', (select json_build_object(
        'bills', count(*), 'sales', coalesce(sum(amt),0),
        ...
        'discounts', coalesce(sum(discount_amount),0), 'service', coalesce(sum(service_amount),0),
        'tax', coalesce(sum(tax_amount),0), 'tips', coalesce(sum(tip_amount),0))
      from (select *, coalesce(nullif(grand_total,0), total_sar+coalesce(delivery_fee,0)) amt
            from exhibitions.table_sessions where tenant_id=v_tenant and status='paid' and closed_at>=v_from and closed_at<v_to) q),
    ...
    'by_day', (select coalesce(json_agg(x order by (x->>'d')),'[]') from (
        select json_build_object('d', closed_at::date, 'sales', sum(coalesce(nullif(grand_total,0),total_sar+coalesce(delivery_fee,0))), 'bills', count(*)) as x
        from exhibitions.table_sessions where tenant_id=v_tenant and status='paid' and closed_at>=v_from and closed_at<v_to
        group by closed_at::date) x),
    ...
    'staff', (select coalesce(json_agg(x),'[]') from (
        ...
        from exhibitions.table_sessions ts left join exhibitions.profiles pr on pr.id=coalesce(ts.closed_by, ts.opened_by)
        where ts.tenant_id=v_tenant and ts.status='paid' and ts.closed_at>=v_from and ts.closed_at<v_to
